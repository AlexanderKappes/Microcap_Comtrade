...
public:
	///имя сигнала
    std::string sig_ch_id;//Имена сигналов перечислены в первой строке
	///фаза сигнала
	char sig_ph;// Последняя буква в названии сигнала
	///привязка сигнала к объекту
    std::string sig_ccbm;// Здесь везде устанавливается ' '
    ...
    ///первичный коэффициент трансформации напряжения или тока
    unsigned int sig_primary;
    ///вторичный коэффициент трансформации напряжения или тока
    unsigned int sig_secondary;
    ///это первичный или вторичный идентификатор масштабирования данных
    unsigned short sig_PS;
	SignalComtr();
	~SignalComtr();
};
